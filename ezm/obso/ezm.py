...
class Zombie(pygame.sprite.Sprite):
    def __init__(self, grid_pos):
        ...
        self.pixel_position = (grid_pos[1] * TILE_SIZE, grid_pos[0] * TILE_SIZE)
        ...
        print(f"New zombie created at grid position: {grid_pos}, pixel position: {self.pixel_position}")
    ...
    def update(self, dt):
        ...
        self.rect.topleft = self.pixel_position
...
class Game:
    ...
    def spawn_zombies(self):
        print(f"Attempting to spawn zombies. Time: {time.time()}, Spawn start time: {self.zombie_spawn_start_time}")
        ...
        if total_zombies < max_zombies:
            ...
            if new_pos:
                new_zombie = Zombie(new_pos)  # Pass the grid position directly
                self.zombies.add(new_zombie)
                print(f"New zombie spawned at grid position: {new_pos}")
            else:
    ...
    def draw_welcome_screen(self):
        ...
        errors = self.validate_settings()
        if errors:
            warning_text = "Invalid settings: " + ", ".join(errors)
    ...
    def check_zombie_collisions(self):
        for zombie in self.zombies:
            if zombie.grid_position == self.player_position:

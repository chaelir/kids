...
def get_best_move(board):
    ...
    for i in range(5):
        for j in range(5):
            if board[i][j] == ' ':
                board[i][j] = 'Green'
                score = minimax(board, 0, False, 3, alpha, beta)
                board[i][j] = ' '
                if score > best_score:
                    best_score = score
                    best_move = (i, j)
    ...
    if is_maximizing:
        ...
        for i in range(5):
            for j in range(5):
                if board[i][j] == ' ':
                    board[i][j] = 'Green'
                    score = minimax(board, depth + 1, False, max_depth, alpha, beta)
                    board[i][j] = ' '
        ...
        for i in range(5):
            for j in range(5):
                if board[i][j] == ' ':
                    board[i][j] = 'Red'
                    score = minimax(board, depth + 1, True, max_depth, alpha, beta)
                    board[i][j] = ' '
